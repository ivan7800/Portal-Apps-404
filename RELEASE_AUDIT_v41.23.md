...
- **Archivo añadido/actualizado:** `RELEASE_AUDIT_v41.23.md`.
...
## Auditoría final de empaquetado (2026-09-12)

- `npm run check`: **22/22 tests superados**, sintaxis JavaScript válida y auditoría local con **96 apps / 118 archivos / 0 avisos / 0 errores**.
- Night Shift 404 está presente en catálogo, tecnologías, portada local, enlaces Pages/GitHub y caché PWA.
- Manifest usa `start_url` y `scope` relativos (`./`) y el service worker limita su interceptación al scope del portal.
- La auditoría online se inició, pero no terminó dentro del límite de ejecución disponible; por tanto, las URLs externas continúan clasificadas como no verificadas en esta sesión.
- Se intentó una prueba de interacción en Chromium headless, pero el navegador aislado no pudo navegar al servidor HTTP local del entorno. No se contabiliza como prueba funcional superada.
- Se corrigió higiene de release: el informe actual ahora se llama `RELEASE_AUDIT_v41.23.md`, README apunta al informe correcto y el changelog coloca v41.23 como entrada más reciente.

### Estado final

**RELEASE CANDIDATE.** No hay bloqueantes detectados por las pruebas automatizadas/locales. Para clasificarla como PUBLICABLE sin reservas falta validar en navegador real el flujo de filtros/vistas/fichas, Safari/iPhone, actualización PWA y la comprobación online de enlaces.
