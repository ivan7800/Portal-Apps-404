...
(function () {
  ...
  if (!D || !Array.isArray(D.APPS)) {
    document.getElementById('app').innerHTML = '<p class=\"fatal\">No se pudo cargar el catálogo.</p>';
    return;
  }
  var APPS = D.APPS;
    ...
    if (q) {
      out = out.filter(function (a) {
        return a.name.toLowerCase().indexOf(q) !== -1 ||
               ...
               a.saga.toLowerCase().indexOf(q) !== -1 ||
               (LANGUAGES[a.name] || '').toLowerCase().indexOf(q) !== -1;
      });
    }
    return out;
  }
  ...
  function render() {
    ...
    '<div class="wrap">' +
      ...
      '<section class="hero" id="top">' +
        '<div>' +
          ...
          '<div class="chips">' +
            ...
            '<span class="chip">Sin tracking</span><span class="chip">PWA instalable</span>' +
          '</div>' +
        ...
        '<div class="oculus">' +
          ...
          '<img src="assets/logo.webp" alt="Símbolo personal de I. Roig: planetas cósmicos" width="512" height="512">' +
        '</div>' +
      '</section>' +
      ...
      '<section class="sec" id="catalogo">' +
        ...
        '<div class="filters">' +
          '<input class="search" id="q" type="search" autocomplete="off" spellcheck="false" placeholder="Buscar apps…" aria-label="Buscar apps" aria-controls="catalog-list" value="' + esc(state.query) + '">' +
        ...
        (list.length
          ? '<div class="grid-mini" id="catalog-list">' + list.map(miniHTML).join('') + '</div>'
    ...
    return '<div class="overlay" id="overlay">' +
      '<div class="modal" id="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-describedby="modal-desc" tabindex="-1">' +
        ...
        '<div class="modal-body">' +
          ...
          '<h3 id="modal-title">' + esc(a.name) + '</h3>' +
          '<p class="modal-desc" id="modal-desc">' + esc(a.description || a.short) + '</p>' +
